...
class MomentReportCreate(BaseModel):
    reason: str
    details: str = ""
...
def _serialize_moment(db: Session, story: Story, viewer: User) -> MomentOut:
    ...
    echo_count = (
        db.query(MomentEcho)
        .filter(MomentEcho.story_id == story.id)
        .count()
    )
    ...
    if story.close_crew_only:
        is_circle = (
            ...
            .filter(
                ...
                Follow.followee_id == story.author_id,
            )
            .first()
            is not None
        )
        ...
        if not is_circle:
            raise HTTPException(
                status_code=403,
                detail="This Moment is only visible to Close Crew",
            )
...
def get_moments_row(
    ...
    stories = (
        db.query(Story)
        .filter(
            Story.author_id.in_(circle_ids),
            (Story.expires_at.is_(None))
    ...
    for author_id, author_stories in grouped.items():
        ...
        serialized = [
            _serialize_moment(db, story, current_user)
        ...
        results.append(
            MomentAuthorGroup(
                ...
                author_avatar_initials=(
                    ...
                    or user_initials(author.display_name)
                ),
                moments=serialized,
                all_viewed=all(
                    moment.viewed for moment in serialized
                ),
            )
        )
...
def get_moment_viewers(
    ...
    """
    Returns the people who viewed a Moment.
    Only the Moment owner can see the viewer list.
    """
    ...
    if story.author_id != current_user.id:
        raise HTTPException(
            status_code=403,
            detail="Only the Moment owner can see viewers",
        )

    rows = (
        db.query(StoryView, User)
        .join(User, User.id == StoryView.viewer_id)
        .filter(StoryView.story_id == moment_id)
    ...
    return [
        {
            ...
            "avatar_initials": (
                ...
                or user_initials(user.display_name)
            ),
...
@router.post("/{moment_id}/report", status_code=201)
def report_moment(
    moment_id: str,
    payload: MomentReportCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    story = _get_moment_or_404(db, moment_id)
    ...
    if story.author_id == current_user.id:
        raise HTTPException(
            status_code=400,
            detail="You cannot report your own Moment",
        )
    ...
    report = Report(
        ...
        target_type=ReportTargetType.story,
        target_id=story.id,
        reason=reason,
...
def list_echoes(
    ...
    return (
        db.query(MomentEcho)
        .filter(MomentEcho.story_id == moment_id)
        .order_by(MomentEcho.created_at.asc())
        .all()
    )
...
@router.post(
    "/{moment_id}/echoes",
    response_model=MomentEchoOut,
    status_code=201,
)
def add_echo(
    moment_id: str,
    payload: MomentEchoCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    story = _get_moment_or_404(db, moment_id)
    ...
    if not story.allow_echo:
        raise HTTPException(
            status_code=403,
            detail="The creator has turned off Echoes for this Moment",
        )
...
def send_on_moment(
    ...
    if not story.allow_send_on:
        raise HTTPException(
            status_code=403,
            detail="The creator has turned off Send On for this Moment",
        )
    ...
    if not membership:
        raise HTTPException(
            status_code=403,
            detail="You're not a member of that Chat",
        )
...
def recast_moment(
    ...
    if original.close_crew_only:
        raise HTTPException(
            status_code=403,
            detail="Private Moments can't be Recast",
        )

    if not original.allow_recast:
        raise HTTPException(
            status_code=403,
            detail="The creator has turned off Recast for this Moment",
        )
...
@router.put(
    "/{moment_id}/permissions",
    response_model=MomentOut,
)
def update_moment_permissions(
    moment_id: str,
    allow_echo: bool = None,
    allow_send_on: bool = None,
    allow_recast: bool = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    story = _get_moment_or_404(db, moment_id)
    ...
    if story.author_id != current_user.id:
        raise HTTPException(
            status_code=403,
            detail="You can only manage your own Moment's permissions",
        )
...
def delete_moment(
    ...
    if story.author_id != current_user.id:
        raise HTTPException(
            status_code=403,
            detail="You can only delete your own Moments",
        )
